...
//    
    val topten = flipped.sortByKey(false).take(10)
    
    for(result<-topten){
    // Look up the name (lookup returns an array of results, so we need to access the first result with (0)).
    val popularName = namesRdd.lookup(result._2)(0)
    
    // Print out our answer!
    println(s"$popularName is a popular superhero with ${result._1} co-appearances.") 
